...
export const PengajuanIzinManager: React.FC<PengajuanIzinManagerProps> = ({
  ...
  const handleApprove = (item: PengajuanIzin) => {
    ...
    dataStorage.updateDatabase((prev) => {
      ...
      // 2. Sinkronkan presensi siswa pada tanggal tersebut
      const currentPresensi = prev.presensi || [];
      const updatedPresensi: PresensiRecord[] = [
        ...currentPresensi.filter(
          (rec) => !(rec.muridId === item.muridId && rec.tanggal === item.tanggal)
        ),
        {
          id: `prs-${item.muridId}-${item.tanggal}`,
                      ...
                      <>
                        ...
                        {role === 'ADMIN' && (
                          <button
                            ...
                            className="p-1.5 text-rose-500 hover:bg-rose-50 rounded-xl text-xs transition"
